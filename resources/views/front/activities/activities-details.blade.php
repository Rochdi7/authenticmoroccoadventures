 ...
 @php
     $seoBase  = trim($activity->title ?? 'Morocco Activity');
     ...
     $seoTitle = mb_strlen($seoBase) + 31 <= 60
         ...
         : $seoBase;
     // Prefix with the title so the description is unique per activity even when two records
     // share boilerplate overview text (identical meta descriptions compete in search).
     $seoBody = trim(strip_tags($activity->overview ?? ''));
     $seoDesc = $seoBody !== ''
...
@php
    ...
    $schema = array_filter([
        ...
        'description' => trim(strip_tags($activity->overview ?? '')) ?: null,
        'url'         => url()->current(),
        'image'       => $seoImage ?: null,
    ...
    if (!empty($activity->duration)) {
        $schema['itinerary'] = ['@type' => 'ItemList', 'name' => $activity->duration];
    }
    if (!empty($activity->location?->name)) {
        $locName = trim($activity->location->name);
                                     ...
                                     @if (!empty($activity->location?->name))
                                         <div class="col-auto">
                                             <div class="d-flex items-center">
                                                 ...
                                                 <a href="{{ route('front.locations.show', $activity->location->slug) }}"
                                                     class="text-inherit">{{ $activity->location->name }}</a>
                                             </div>
                                         </div>
                                     @endif
